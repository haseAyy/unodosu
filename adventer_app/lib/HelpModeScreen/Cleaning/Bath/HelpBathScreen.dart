...
//おふろ問題出題画面
class HelpBathScreen extends StatefulWidget {
  final int questionCount; //問題カウント
  final int correctCount; //正解数カウント

  const HelpBathScreen(
      {required this.questionCount, required this.correctCount});

  @override
  _HelpBathScreenState createState() =>
      _HelpBathScreenState(questionCount, correctCount);
}

class _HelpBathScreenState extends State<HelpBathScreen> {
  late Future<Question?> questionFuture;
  late int questionCount; //このクラス内で管理する変数
  late int correctCount; //正解数を追跡する変数

  //コンストラクタで初期値を設定
  _HelpBathScreenState(this.questionCount, this.correctCount);

  @override
  void initState() {
    super.initState();
    questionFuture = fetchQuestion("おふろ"); // questiontypeTemeを指定
  }
                    ...
                    child: Column(
                      children: [
                        ...
                        Container(
                          ...
                          height: screenSize.width * 0.5, // 比率調整
